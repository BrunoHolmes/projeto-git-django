...
def editar(request):
    if request.method == 'POST':
        ...
        cliente = Clientes.objects.filter(id=user_id)
        cliente.update(nome=nome, cpf=cpf, endereco=endereco, numero=numero, complemento=complemento, bairro=bairro, municipio=municipio, estado=estado, pais=pais, email=email)
